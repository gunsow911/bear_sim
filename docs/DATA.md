...
県全域905件の月別分布を平均=1に正規化した「月形状」を、16ターン=**7月〜10月**の窓へ
割り当て、2点アフィン写像で **7月開始を活発度10・10月ピークを80** に対応づける（turn1=7月初旬
=10、turn16=10月末=80）。開始を低くしておき、イベントで上下の補正が加わる前提。

月形状（生値）：7月1.101 / 8月0.835 / 9月1.485 / 10月2.533。
...
判断：ゲームの「活発度」は終盤の荒食い期へ向けて単調に高まる緊張を表すため、8月の谷は
**累積最大でならして 7月→10月を非減少にする**（秋ピークは残す）。`season.ts` は生値
`MONTH_SHAPE_RAW` を持ち、コード内で単調化する。結果のカーブ（turn1..16）はおおよそ
`[10,10,10,10,10,10,14,18,21,25,29,39,49,60,70,80]`（10で開始しプラトー後、秋に急伸）。
